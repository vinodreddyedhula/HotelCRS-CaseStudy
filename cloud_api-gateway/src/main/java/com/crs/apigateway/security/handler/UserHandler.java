 ...
 * 
 * import org.springframework.web.reactive.function.BodyInserters; import
 ...
 * 
 * public UserHandler(UserRepository userRepository){ this.userRepository =
 * userRepository; }
 * 
 * public Mono<ServerResponse> createUser(ServerRequest request) { Mono<User>
 * userMono = request.bodyToMono(User.class).flatMap(user ->
 * userRepository.save(user)); return
 * ServerResponse.ok().contentType(APPLICATION_JSON).body(userMono, User.class);
 * }
 * 
 * public Mono<ServerResponse> listUser(ServerRequest request) { Flux<User> user
 * = userRepository.findAll(); return
 * ServerResponse.ok().contentType(APPLICATION_JSON).body(user, User.class); }
 * 
 ...
 * ServerResponse.notFound().build(); Mono<User> userMono =
